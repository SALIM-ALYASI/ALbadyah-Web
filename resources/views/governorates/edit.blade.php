...
@section('content')
    ...
    <div class="flex flex-wrap items-center justify-between gap-4 mb-6">
        <div>
            <h1 class="m-0 text-2xl font-bold text-ab-navy">تعديل المحافظة</h1>
            <p class="m-0 mt-1 text-sm text-ab-body">تعديل بيانات المحافظة: {{ $governorate->name_ar }}</p>
        </div>
        <div class="flex items-center gap-2">
            <a href="{{ route('governorates.show', $governorate->id) }}"
                ...
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-7 11-7 11 7 11 7-4 7-11 7-11-7-11-7Z"></path><circle cx="12" cy="12" r="3"></circle></svg>
                عرض التفاصيل
            </a>
    ...
    <div class="max-w-2xl mx-auto bg-white border border-ab-border rounded-[22px] p-6 sm:p-8">
        <form action="{{ route('governorates.update', $governorate->id) }}" method="POST" enctype="multipart/form-data" class="flex flex-col gap-5">
            @csrf
            @method('PUT')
            ...
            <div class="grid gap-5" style="grid-template-columns:repeat(auto-fit, minmax(220px,1fr))">
                ...
                <label class="flex flex-col gap-1.5">
                    ...
                    <input type="text" name="name_en" value="{{ old('name_en', $governorate->name_en) }}" required dir="ltr"
            ...
            <label class="flex flex-col gap-1.5">
                ...
                <input type="url" name="website_url" value="{{ old('website_url', $governorate->website_url) }}" dir="ltr"
            ...
            <x-admin.image-upload name="image" url-name="image_url" label="صورة المحافظة"
                :current="$governorate->has_image ? $governorate->image_url : null" />
            ...
            <div class="flex items-center justify-end gap-3 pt-4 border-t border-ab-border">
                ...
                <button type="submit" class="px-6 py-2.5 rounded-full bg-ab-navy text-white text-sm font-semibold">حفظ التعديلات</button>
            </div>
        </form>
    </div>
