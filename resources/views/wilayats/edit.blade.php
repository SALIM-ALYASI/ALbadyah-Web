...
@section('content')
    ...
    <div class="flex flex-wrap items-center justify-between gap-4 mb-6">
        <div>
            <h1 class="m-0 text-2xl font-bold text-ab-navy">تعديل الولاية</h1>
            <p class="m-0 mt-1 text-sm text-ab-body">تعديل بيانات الولاية: {{ $wilayat->name_ar }}</p>
        </div>
    ...
    <div class="max-w-2xl mx-auto bg-white border border-ab-border rounded-[22px] p-6 sm:p-8">
        <form action="{{ route('wilayats.update', $wilayat->id) }}" method="POST" id="wilayatForm" enctype="multipart/form-data" class="flex flex-col gap-5">
            @csrf
            @method('PUT')
            ...
            <div class="grid gap-5" style="grid-template-columns:repeat(auto-fit, minmax(220px,1fr))">
                <label class="flex flex-col gap-1.5">
                    ...
                    <input type="text" name="name_ar" value="{{ old('name_ar', $wilayat->name_ar) }}" required
                ...
                <label class="flex flex-col gap-1.5">
                    ...
                    <input type="text" name="name_en" value="{{ old('name_en', $wilayat->name_en) }}" required dir="ltr"
            ...
            <label class="flex flex-col gap-1.5">
                ...
                <select name="governorate_id" required
                    ...
                    @foreach ($governorates as $governorate)
                        <option value="{{ $governorate->id }}" @selected(old('governorate_id', $wilayat->governorate_id) == $governorate->id)>{{ $governorate->name_ar }}</option>
                    @endforeach
                </select>
            ...
            <label class="flex flex-col gap-1.5">
                ...
                <input type="url" name="website_url" value="{{ old('website_url', $wilayat->website_url) }}" dir="ltr"
            ...
            <x-admin.image-upload name="image" url-name="image_url" label="صورة الولاية (الحد الأقصى 2 ميجابايت)"
                :current="$wilayat->has_image ? $wilayat->image_url : null" />
