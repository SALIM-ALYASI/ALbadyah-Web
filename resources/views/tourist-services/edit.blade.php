        ...
        <div class="card">
            ...
            <div class="card-body">
                <form action="{{ route('tourist-services.update', $touristService->id) }}" method="POST" id="editForm">
                    ...
                    <div class="mb-4 p-3 border rounded {{ $touristService->is_active ? 'border-success bg-success bg-opacity-10' : 'border-warning bg-warning bg-opacity-10' }}">
                        <div class="form-check form-switch">
                            <input class="form-check-input" type="checkbox"
                                   id="is_active" name="is_active" value="1"
                                   {{ old('is_active', $touristService->is_active) ? 'checked' : '' }}>
                            <label class="form-check-label fw-bold" for="is_active">
                                <i class="fas fa-globe me-1"></i>
                                نشر الخدمة (تظهر بالموقع العام)
                            </label>
                        </div>
                        <small class="text-muted d-block mt-1">
                            الحالة الحالية:
                            {{ $touristService->is_active ? 'منشورة' : 'غير منشورة' }} —
