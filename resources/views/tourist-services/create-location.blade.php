...
@section('content')
    ...
    <div class="flex flex-wrap items-center justify-between gap-4 mb-6">
        <div>
            <h1 class="m-0 text-2xl font-bold text-ab-navy">إضافة موقع خدمة سياحية</h1>
            <p class="m-0 mt-1 text-sm text-ab-body">أدخل بيانات الموقع الأساسية للخدمة السياحية</p>
        </div>
    ...
    <div class="max-w-2xl mx-auto bg-white border border-ab-border rounded-[22px] p-6 sm:p-8">
        <form action="{{ route('tourist-services.store-location') }}" method="POST" enctype="multipart/form-data" id="locationForm" class="flex flex-col gap-5">
            @csrf

            <div class="grid gap-5" style="grid-template-columns:repeat(auto-fit, minmax(220px,1fr))">
                <label class="flex flex-col gap-1.5">
                    <span class="text-sm font-semibold text-ab-navy">اسم الموقع بالعربية *</span>
                ...
                <label class="flex flex-col gap-1.5">
                    <span class="text-sm font-semibold text-ab-navy">اسم الموقع بالإنجليزية *</span>
            ...
            <div class="grid gap-5" style="grid-template-columns:repeat(auto-fit, minmax(180px,1fr))">
                <label class="flex flex-col gap-1.5">
                    <span class="text-sm font-semibold text-ab-navy">نوع الخدمة الرئيسي (اختياري)</span>
            ...
            <x-admin.image-upload name="location_image" url-name="location_image_url" label="صورة الموقع (اختياري)" />
            ...
            <div class="flex items-center justify-end gap-3 pt-4 border-t border-ab-border">
                ...
                <button type="submit" id="submitBtn" class="px-6 py-2.5 rounded-full bg-ab-navy text-white text-sm font-semibold">حفظ الموقع والمتابعة لإضافة الخدمات</button>
            </div>
        </form>
    </div>
...
<script>
    document.getElementById('locationForm').addEventListener('submit', function () {
        ...
        setTimeout(() => { btn.disabled = false; btn.textContent = 'حفظ الموقع والمتابعة لإضافة الخدمات'; }, 3000);
    });
</script>
@endpush
