...
@php
    $sitesCount = $wilayat->tourist_sites_count ?? 0;
    $servicesCount = $wilayat->tourist_services_count ?? 0;
    $hasContent = $sitesCount > 0 || $servicesCount > 0;
@endphp

<a href="{{ route('tourism.wilayat', $wilayat->slug ?: $wilayat->id) }}"
    ...
    <div class="relative aspect-[4/3] bg-ab-cool">
        @if ($wilayat->has_image)
            <img src="{{ $wilayat->image_url }}" alt="{{ $wilayat->name_ar }}" loading="lazy"
    ...
    <div class="flex flex-col gap-3 p-5 flex-1">
        <h3 class="m-0 text-2xl font-bold text-ab-navy">{{ $wilayat->name_ar }}</h3>
